...
class VkBot:
    def __init__(self) -> None:
        ...
        Конструктор
        """
        self.vk_session = vk_api.VkApi(token='debb517afd6cc884e0448d4a176e92499837f321adc503d287fb2f9822d784e208317bf5ed912e7202bd7')
    ...
    def _show_5days_weather(self, user_id: int) -> None:
            api_key = '72afaf978f62b09941ff799311282463'
            api_call = 'https://api.openweathermap.org/data/2.5/forecast?appid=' + api_key+'&lang=ru'
            ...
            font = ImageFont.truetype(font='lato.ttf', size=18)
            # title2 = ImageFont.truetype(size=30)
            idraw.text((25, 10), 'Погода в Москве', font=title, fill="white")
            self._send_message(user_id,a)
            while running:
                ...
                print('Москва'.format(**location_data))
                ...
                for item in json_data['list']:
                    ...
                    img_name = item['weather'][0]['icon']
                    im = Image.open(urlopen('https://openweathermap.org/img/wn/{}@4x.png'.format(img_name)))
                    #img.paste(im, (120, 20), im.convert('RGBA'))
                    
                    print('Погода: %s' % description)
                    b = 'Погода: %s' % description
                    print('Градусов на улице: {:.2f}'.format(temperature - 273))
                    
                    c='Градусов на улице: {:.2f}'.format(temperature - 273)
                    allw=p+"\n"+k+"\n"+b+"\n"+c
                    #idraw.text((10, 60), allw, font=font, fill="white")
                    #img.save('data/weather_card.png')

                    #photo = upload.photo_messages('data/weather_card.png')[0]
                    #attachments.append("photo{}_{}".format(photo["owner_id"], photo["id"]))
                    #self._send_message_with_attachments(user_id=user_id, text='Москва 5 дней', attachments=attachments)
                    #img.delete('data/weather_card.png')
                    self._send_message(user_id,allw)
                    self._send_message_with_attachments(img_name)
                    while True:
                     running = False
                     break
    ...
    def _show_today_weather(self, user_id: int) -> None:
                    ...
                    pressure = str(int(float(info['main']['pressure']) / 1.33))
                    humidity = str(info['main']['humidity'])
                    wind_speedi = info['wind']['speed']
                    wind_speed = round(wind_speedi+2)
                    wind_slug = self._get_wind_slug(float(wind_speed)).lower()
                    wind_deg_slug = self._get_wind_deg_slug(info['wind']['speed'])
                    weather = '{}, температура: {}°C\nДавление: {} мм рт. сб., влажность: {}%\nВетер: {}, {} м/с, {}'.format(status, temp,pressure, humidity, wind_slug, wind_speed,wind_deg_slug)
                    idraw.text((10, 185), weather, font=font, fill="white")
                    img.save('data/weather_card.png')
                    ...
                    self._send_message_with_attachments(user_id=user_id, text='Сегодня', attachments=attachments)
    ...
    def _show_help_message(self, user_id: int) -> None:
        ...
        self._send_message(user_id, 'Список команд:\n\nНачать - запуск\nБот - расписание\n' \
            'Бот <номер группы> - Запоминает группуу\n' \
            ...
            'Найти <фамилия преподавателя> - Получить расписание преподавателя за определённый период\n\n' \
            '<> - Обязательные аргументы\n'\
            'Погода - показывает вам погоду\n'\
            'корона - показывает корону в России\n'\
            'корона <> - корона в какой-то области')
